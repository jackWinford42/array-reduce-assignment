...
function vowelCount(str) {
    ...
    return str.toLowerCase().split('').reduce(function(accum, nextletter) {
        if (vowels.includes(nextletter)){
            if (accum[nextletter] === undefined) {
                accum[nextletter] = 1;
            } else {
                accum[nextletter]++;
            }
...
function partition(arr, callback) {
    ...
    return arr.reduce(function(accum, item) {
        if (callback(item)) {
            accum[0].push(item);//[firstIndex] = item;
            firstIndex++;
        } else {
            accum[1].push(item);//[secondIndex] = item;
            secondIndex++;
        }
        return accum;
    },[[],[]]);
